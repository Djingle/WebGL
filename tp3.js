...
void main()
{
    ...
    pos = vec3(uProjectionMatrix * uViewMatrix * uModelMatrix * vec4(position_in, 1.0));
    normal = vec3(uProjectionMatrix * uViewModelMatrix * vec4(normal_in, 1.0));
...
void main()
{
    ...
    vec3 lightDirection;
    lightDirection = normalize(pos);
    float terme;
    terme = max(0.0, (normalN.dot(lightDirection)) / (length(normalN) * length(lightDirection)));
    float ia = uLightIntensity * uKa;
    float id = uLightIntensity * uKd * terme;
...
function SceneManager_add( object )
{
    ...
    let normal_positions = new Float32Array(object.vertices, object.indices);
    let vbo_normals = gl.createBuffer();
    gl.bindBuffer(gl.ARRAY_BUFFER, vbo_normals);
    gl.bufferData(gl.ARRAY_BUFFER, normal_positions, gl.STATIC_DRAW);
    gl.bindBuffer(gl.ARRAY_BUFFER, null);
...
function init_wgl()
{
	...
	let dataPath = 'nissan-gtr/part'; // BEWARE: the "models" directory HAVE TO be placed in the "tp" directory near your javascript file for the TP
...
function draw_wgl()
{
    ...
    let viewmodel = Matrix.mult(viewMatrix, modelMatrix);
    Uniforms.uViewModelMatrix = viewmodel.inverse3transpose();

    Uniforms.uLightIntensity = (100, 100, 100);
    Uniforms.uLightPosition = (0, 0, 0);
