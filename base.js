...
void main()
{
    ...
    gl_Position = projMat * viewMat * modelMat * vec4(position_in, 0.0, 1.0);
}
`;
...
function init_wgl()
{
    ...
    let len = (imax - 1) * (jmax - 1) * 6;
    let data_indices = new Uint8Array(len);
    for (let j = 0; j < len/6; j++) {
        //triangle du haut
        data_indices[j] = j;
        data_indices[j+1] = j+1;
        data_indices[j+2] = j+imax+1;
        //triangle du bas
        data_indices[j+3] = j+imax+1;
        data_indices[j+4] = j+imax;
        data_indices[j+5] = j;
    }
...
function draw_wgl()
{
    ...
    //ewgl.scene_camera.get_view_matrix() pour régler à la souris la caméra.

    
    Uniforms.modelMat = Matrix.rotateZ(30);
	...
	// Draw commands
	gl.drawElements(gl.LINE_STRIP, 3, gl.UNISIGNED_SHORT, 0);
